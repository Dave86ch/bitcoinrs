...
pub enum Message {
    /// Fecth all UTXOs belonging to a public key
    ...
    // block (e.g. if one as been found in the meantime),
    ...
    DiscoverNodes,
    // This is the reponse of the DiscoverNodes
    NodeList(Vec<String>),
...
// And we are going to use ciborium (CBOR) for serializzation
impl Message {
    ...
    pub fn decode(data: &[u8]) -> Result<Message, ciborium::de::Error<IoError>> {

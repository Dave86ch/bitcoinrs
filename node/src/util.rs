...
pub async fn find_longest_chain_node() -> Result<(String, u32)> {
    println!("finding the nodes with the longest blockchain length...");
    ...
    for node in all_nodes {
        ...
        match message {
            Message::Difference(count) => {
                println!("received diffrence from {}", node);
...
pub async fn load_blockchain(blockchain_file: &str) -> Result<()> {
    println!("Blockchain file exits, loading...");
    ...
    *blockchain = new_blockchain;
    ...
    println!("checking if the target needs to be adjusted...");

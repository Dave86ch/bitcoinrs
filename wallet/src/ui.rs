...
// Set up the menubar with "Send" and "Quit" options
...
fn create_info_layout(core: &Arc<Core>) -> LinearLayout {
    ...
    let keys_content = core
        ...
        .my_keys
        .iter()
        .map(|key| format!("{}", key.private.dispaly()))
        .collect::<Vec<String>>()
        .join("\n");
...
// Dispaly a successs dialog after a successful transaction
...
// Dispaly an error dialog when a transaction fails
